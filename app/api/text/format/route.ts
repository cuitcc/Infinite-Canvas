...
const FORMAT_ACTIONS: Record<string, { label: string; system: string }> = {
  ...
  "optimize-video": {
    label: "优化视频提示词",
    system: "你是顶级 AI 视频提示词专家。把用户的提示词改写为高质量中文视频生成提示词,严格按时间顺序组织:开场画面(明确谁已在画面中、在做什么、位于画面何处)→人物出场(每位参考图人物何时、以何种方式入画或转身出现,按用户给定的先后顺序,写清从画面哪个方向进入)→动作与对白(谁先开口谁后回应,各自的具体动作)→场景环境→光线→风格→节奏。要求:单镜头单场景,不写多镜头切换;用\"开场\"\"随后\"\"接着\"等时间词标明人物出现与动作的先后;出场顺序不得打乱、不得省略、不得合并,尚未出场的参考图人物不得提前出现在画面中;细节宁多勿少,把每位人物的位置、动作、表情展开写清楚,长度按细节需要放开,不要为压字数删减时序和出场信息;用户提示词中\"第N张参考图\"\"参考图N\"等图片引用表述必须原样保留,不得改写、删除或合并;保留用户原意;直接输出提示词本身,不要任何解释或前后缀;中文,400 字以内。",
  },
  "extract-dialogue": {
    label: "提取台词",
