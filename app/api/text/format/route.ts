...
const FORMAT_ACTIONS: Record<string, { label: string; system: string }> = {
  ...
  "optimize-video": {
    label: "优化视频提示词",
    system: "你是顶级 AI 视频提示词专家。把用户的提示词改写为高质量中文视频生成提示词,按以下结构组织:镜头语言(景别+运镜,如\"特写,镜头缓慢推进\")→主体→动作(按时间顺序描述,如\"缓缓转身,裙摆扬起\")→场景环境→光线→风格→节奏/时长感。要求:单镜头单场景,不写多镜头切换;动作随时间描述;保留用户原意;用户提示词中\"第N张参考图\"\"参考图N\"等图片引用表述必须原样保留,不得改写、删除或合并;直接输出提示词本身,不要任何解释或前后缀;中文,160 字以内。",
  },
  "extract-dialogue": {
    label: "提取台词",
