...
{"title":"片名","genre":"类型","synopsis":"100字内剧情梗概","characters":[{"name":"角色名","appearance":"外貌/年龄/服装细节,40字内,用于生成角色立绘"}],"scenes":[{"name":"场景名","description":"场景视觉描述,30字内"}],"script":"分场剧情与对白全文,每行格式 角色名：台词,900字内"}
...
4.description 中的'<Picture N>'编号按大纲角色列表的全局顺序从1编号(第一个角色永远是<Picture 1>,与本镜出场顺序无关);characters 数组仍按本镜出场顺序填写;dialogue 必须逐字取自剧本对白(保留原句,禁止改写、缩写或把两句合并成一句),按剧本对白顺序依次分配到各镜,每行说话人必须是 characters 数组中的人物,每镜最多2句且说话人不同,可空数组;台词量必须与每镜时长匹配(输入的 secondsPerShot,秒):按每句台词约4秒估算,6秒及以下的镜头最多1句,台词务必短促;装不下的剧本对白,其关键动作信息(括号内的动作神态)并入该镜 description;道具出镜时 description 必须写出道具名称(与可用道具资产名一致);
