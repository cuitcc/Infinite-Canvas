...
要求:2-3个角色,其中必须有对立面(反派/竞争者/制造冲突的角色),2-3个场景;剧情必须有明确的矛盾冲突和转折,开头即抛出悬念钩子,结尾留钩子;对白短促,每句台词控制在15字以内、有攻防和潜台词,一方施压一方应对,禁止长篇独白、禁止旁白解说;每个有名字的角色至少2句台词;台词总量必须匹配片长:用户消息末尾会给出全片时长(分镜数×每镜秒数),全部对白按每句约4秒估算,总说话时长不得超过全片时长的70%,超出时精简台词句数、保留冲突最尖锐的句子,确保分镜阶段一句都不用丢。`,
...
4.description 中的'<Picture N>'编号按大纲角色列表的全局顺序从1编号(第一个角色永远是<Picture 1>,与本镜出场顺序无关);characters 数组仍按本镜出场顺序填写;dialogue 必须逐字取自剧本对白(保留原句,禁止改写、缩写或把两句合并成一句),按剧本对白顺序依次分配到各镜,每行说话人必须是 characters 数组中的人物,可空数组;台词与剧本对白必须100%对齐(硬性要求):所有分镜的 dialogue 按顺序拼接后必须与剧本对白逐句一一对应,禁止遗漏、丢弃或跳过任何一句,宁可某镜没有台词也不能丢句;每镜不设句数上限,但台词量要与该镜时长匹配(用户消息中的每镜秒数,按每句约4秒估算):某镜装不下时把装不下的台词整体顺延到下一镜,不得为了塞进本镜而删句;被顺延台词的关键动作信息(括号内的动作神态)并入该镜 description;道具出镜时 description 必须写出道具名称(与可用道具资产名一致);description 中严禁出现台词原文(禁止"角色名：台词"格式,禁止引号台词),角色说话只写动作与神态(如"开口质问""冷笑开口"),台词原文只允许出现在 dialogue 数组,同一句台词绝不允许在 description 和 dialogue 中重复出现;
